...
void Graphics::RenderFrame()
{
    ...
    //Draw Text
    spriteBatch->Begin();
    spriteFont->DrawString(spriteBatch.get(), L"Render Engine", DirectX::XMFLOAT2(0,0),
...
{
    std::vector<AdapterData> adapters = AdapterReader::GetAdapters();//find all Graphiccard
    if (adapters.size() < 1)
    {
    ...
    if (FAILED(hr))
    {
        ErrorLogger::Log(hr, "Failed to get BackBuffer of the swapchain");
        return false;
    }
    ...
    spriteFont = std::make_unique<DirectX::SpriteFont>(this->deviceContext.Get(), L"Data/Fonts/comic_sans_ms_16.spritefont");
